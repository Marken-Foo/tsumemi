...
class BoardSkin(Enum):
    ...
    MILITARY = (
        "Military",
        "#a06b3a",
        os.path.join(BOARD_IMAGES_PATH, r"tile_military.png"),
    )
    MILITARY2 = (
        "Military2",
        "#bd7b32",
        os.path.join(BOARD_IMAGES_PATH, r"tile_military2.png"),
    )

    def __init__(self, desc: str, colour: str, path: PathLike) -> None:
        self.desc: str = desc
...
class PieceSkin(Enum):
    ...
    REDWOOD = (
        "1-kanji red wood pieces",
        os.path.join(PIECE_IMAGES_PATH, r"kanji_red_wood"),
    )
    INTL = (
        "Internationalised symbols",
        os.path.join(PIECE_IMAGES_PATH, r"international"),
    )
    TOMATO_COLOURED = (
        "CouchTomato internationalised symbols",
        os.path.join(PIECE_IMAGES_PATH, r"tomato_colored"),
    )
    TOMATO_MONOCHROME = (
