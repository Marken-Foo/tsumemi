...
class TestMoveGeneration(unittest.TestCase):
    ...
    def test_pawn_moves(self):
        ...
        mvlist_sente = rules.generate_valid_moves(pos=self.position, side=Side.SENTE, ktype=KomaType.FU)
        mvlist_gote = rules.generate_valid_moves(pos=self.position, side=Side.GOTE, ktype=KomaType.FU)
    ...
    def test_lance_moves(self):
        ...
        self.position.from_sfen(sfen)
        mvlist_sente = rules.generate_valid_moves(pos=self.position, side=Side.SENTE, ktype=KomaType.KY)
        mvlist_gote = rules.generate_valid_moves(pos=self.position, side=Side.GOTE, ktype=KomaType.KY)
        mvset_sente = set((move.to_latin() for move in mvlist_sente))
        mvset_gote = set((move.to_latin() for move in mvlist_gote))
        # check answers
        self.assertEqual(mvset_sente, set(sente_moves))
    ...
    def test_knight_moves(self):
        ...
        self.position.from_sfen(sfen)
        mvlist_sente = rules.generate_valid_moves(pos=self.position, side=Side.SENTE, ktype=KomaType.KE)
        mvlist_gote = rules.generate_valid_moves(pos=self.position, side=Side.GOTE, ktype=KomaType.KE)
        mvset_sente = set((move.to_latin() for move in mvlist_sente))
        mvset_gote = set((move.to_latin() for move in mvlist_gote))
        # check answers
        self.assertEqual(mvset_sente, set(sente_moves))
    ...
    def test_silver_moves(self):
        ...
        self.position.from_sfen(sfen)
        mvlist_sente = rules.generate_valid_moves(pos=self.position, side=Side.SENTE, ktype=KomaType.GI)
        mvlist_gote = rules.generate_valid_moves(pos=self.position, side=Side.GOTE, ktype=KomaType.GI)
        mvset_sente = set((move.to_latin() for move in mvlist_sente))
        mvset_gote = set((move.to_latin() for move in mvlist_gote))
        # check answers
        self.assertEqual(mvset_sente, set(sente_moves))
    ...
    def test_gold_moves(self):
        ...
        self.position.from_sfen(sfen)
        mvlist_sente = rules.generate_valid_moves(pos=self.position, side=Side.SENTE, ktype=KomaType.KI)
        mvlist_gote = rules.generate_valid_moves(pos=self.position, side=Side.GOTE, ktype=KomaType.KI)
        mvset_sente = set((move.to_latin() for move in mvlist_sente))
        mvset_gote = set((move.to_latin() for move in mvlist_gote))
        # check answers
        self.assertEqual(mvset_sente, set(sente_moves))
    ...
    def test_bishop_moves(self):
        ...
        self.position.from_sfen(sfen)
        mvlist_sente = rules.generate_valid_moves(pos=self.position, side=Side.SENTE, ktype=KomaType.KA)
        mvlist_gote = rules.generate_valid_moves(pos=self.position, side=Side.GOTE, ktype=KomaType.KA)
        mvset_sente = set((move.to_latin() for move in mvlist_sente))
        mvset_gote = set((move.to_latin() for move in mvlist_gote))
        # check answers
        self.assertEqual(mvset_sente, set(sente_moves))
    ...
    def test_rook_moves(self):
        ...
        self.position.from_sfen(sfen)
        mvlist_sente = rules.generate_valid_moves(pos=self.position, side=Side.SENTE, ktype=KomaType.HI)
        mvlist_gote = rules.generate_valid_moves(pos=self.position, side=Side.GOTE, ktype=KomaType.HI)
        mvset_sente = set((move.to_latin() for move in mvlist_sente))
        mvset_gote = set((move.to_latin() for move in mvlist_gote))
        # check answers
        self.assertEqual(mvset_sente, set(sente_moves))
    ...
    def manual_test_drop_moves(self):
        ...
        # answer keys
        self.position.from_sfen(sfen)
        droplist_fu = rules.generate_drop_moves(pos=self.position, side=Side.SENTE, ktype=KomaType.FU)
        droplist_ke = rules.generate_drop_moves(pos=self.position, side=Side.SENTE, ktype=KomaType.KE)
        droplist_ka = rules.generate_drop_moves(pos=self.position, side=Side.SENTE, ktype=KomaType.KA)
        # check answers
